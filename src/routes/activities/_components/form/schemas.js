...
const baseValidation = {
	...
	shortDescription: yup.string().ensure().trim().required('Please add a short description.'),
...
const scheduleValidation = {
	...
	selectedTimezone: yup.string().required('Please select the appropiate timezone.'),
	selectedDuration: yup.string().required('Please select an estimated duration.')
};
...
const extendedValidation = {
	/* 
		The What
	*/
	longDescription: yup.string().ensure().trim().required('Please add a full description.'),

	/*
		Activity Attributes
	*/
	targetLocation: yup.string().ensure().required('Please select your desired location.'),

	category: yup.string().ensure().trim().required('Please add a target track.'),

	type: yup.string().ensure().trim().required('Please select what type of activity this is.'),
	...
	/* 
		target audience
	*/
	targetAudience: yup.array().of(yup.string()).min(1, 'Please select your intended audience.'),
	...
	*/
	...
	mentorship: yup.string().required('Please tell us if you need any help preparing.'),

	canRecord: yup.boolean().required('Please tell us if you would like to be recoreded.'),

	isMinor: yup.boolean().required('Please indicate if you are under 18 years old.'),
...
const workshopValidation = {
	/*
		Workshop Details
	*/
	agenda: yup.string().ensure().trim().required('Please add an agenda category.'),

	duration: yup.string().ensure().required('Please select the appromiate duration.'),
