    ...
    registrations.forEach(r => {
      switch (r.product.name.toUpperCase()) {
        ...
        case 'CAMPER NO FOOD':
          ...
          statsCamperCheckedIn = r.hasCheckedIn
            ? statsCamperCheckedIn + 1
            : statsCamperCheckedIn;
          break;
  ...
  <div class="mt-12 flex flex-col justify-center">
    <div class="flex justify-evenly">
      <SemiCircle
        title="Counselors"
        series="{(
          (statsCounselorCheckedIn / statsCounselorTotal) *
          100
        ).toFixed()}" />
      <SemiCircle
        title="Campers"
        series="{(
          (statsCamperCheckedIn / statsCamperTotal) *
          100
        ).toFixed()}" />
      <SemiCircle
        title="Virtual Campers"
        series="{(
          (statsVirtualCamperCheckedIn / statsVirtualCamperTotal) *
          100
        ).toFixed()}" />
      <SemiCircle
        title="Camp Swag"
        series="{(
          (statsCampSwagCheckedIn / statsCampSwagTotal) *
          100
        ).toFixed()}" />
      <SemiCircle
        title="Pig Roast"
        series="{(
          (statsPigRoastCheckedIn / statsPigRoastTotal) *
          100
        ).toFixed()}" />
    </div>
  </div>
