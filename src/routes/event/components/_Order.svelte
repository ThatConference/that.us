          ...
          <div class="hidden md:block">
            <div>
              <p class="flex text-sm text-gray-900">
                Purchased On:
                <time
                  class="pl-2 font-extrabold"
                  datetime="{registration.orderDate}">
                  {dayjs(registration.orderDate).format(
                    'dddd, MMMM D, YYYY - h:mm A',
                  )}
                </time>
              </p>
              <div
                class="mt-2 flex items-center text-sm text-gray-500 space-x-2">
                <span
                  class="rounded-full py-1 px-8 bg-green-300  text-white font-extrabold">
                  {registration.orderAllocations.length}
                </span>
                <span>
                  {registration.orderAllocations.length > 1
                    ? 'tickets'
                    : 'ticket'}
                </span>
              </div>
