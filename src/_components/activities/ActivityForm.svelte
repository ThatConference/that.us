...
<script>
	...
	const schema = yup.object().shape({
		...
		shortDescription: yup.string().trim().required('Please add a short description.'),
	...
	const canCancelActivityAction = () => {
		...
		if (handleWithdraw) {
			if (dayjs(initialValues.startTime).isSameOrAfter(dayjs(), 'day')) results = true;
		}
		return results;
	};
	...
	const findSelectedDuration = (values) =>
		estimatedDurationOptions.find((item) => item.value === values['selectedDuration']);
	...
	$: timeSlotOptionsFiltered = dayjs(selectedDateValue).isToday()
		? timeSlotOptions.filter((t) => dayjs(dayjs(t.value, 'HH:mm')).isSameOrAfter(dayjs()))
		: timeSlotOptions;
</script>
...
>
	<div>
		...
		>
          ...
          sm:pt-2"
			>
				Set 5 or less 'tags' to help categorize this chat. (return/enter is the delimiter)
			</label>
			<div class="mt-1 sm:mt-0 sm:col-span-2">
